...
Blockly.Python.finish = function(a){var b = [],c = [],d;for(d in Blockly.Python.definitions_){if(check(d)){var e = Blockly.Python.definitions_[d];e.match(/^(from\s+\S+\s+)?import\s+\S+/)?b.push(e):c.push(e)}}delete Blockly.Python.definitions_;delete Blockly.Python.functionNames_;Blockly.Python.variableDB_.reset();return(b.join("\n")+"\n\n"+c.join("\n\n")).replace(/\n\n+/g,"\n\n").replace(/\n*$/,"\n\n\n")+a};Blockly.Python.scrubNakedValue = function(a){return a+"\n"};
...
if("GET"  ===  b){return[d+"[-1]",Blockly.Python.ORDER_MEMBER];}if("GET_REMOVE"  ===  b)
{return[d+".pop()",Blockly.Python.ORDER_FUNCTION_CALL];}
if("REMOVE"  ===  b){return d+".pop()\n";};break;
case "FROM_START":a = Blockly.Python.getAdjustedInt(a,"AT");if("GET"  ===  b){
...
case "FROM_END":a = Blockly.Python.getAdjustedInt(a,"AT",1,!0);
if("GET"  ===  b){return[d+"["+a+"]",Blockly.Python.ORDER_MEMBER];}if("GET_REMOVE"  ===  b){return[d+".pop("+a+")",
Blockly.Python.ORDER_FUNCTION_CALL];}if("REMOVE"  ===  b){return d+".pop("+a+")\n";};
break;case "RANDOM":Blockly.Python.definitions_.import_random = "import random";
...
if("GET_REMOVE"  ===  b){return[d,Blockly.Python.ORDER_FUNCTION_CALL];}if("REMOVE"  ===  
b){return d+"\n"}throw"Unhandled combination (lists_getIndex).";break;default:}
...
switch(d){
case "FIRST":if("SET"  ===  c){return b+"[0]  =  "+e+"\n";}if("INSERT"  ===  c){return b+".insert(0, "+e+")\n";}break;
case "LAST":if("SET"  ===  c){return b+"[-1]  =  "+e+"\n";}if("INSERT"  ===  c){return b+".append("+e+")\n";}break;
case "FROM_START":a = Blockly.Python.getAdjustedInt(a,"AT");if("SET"  ===  c){return b+"["+a+"]  =  "+e+"\n";}if("INSERT"  ===  c){return b+".insert("+a+", "+e+")\n";}break;
case "FROM_END":a = Blockly.Python.getAdjustedInt(a,"AT",1,!0);if("SET"  ===  c){return b+"["+a+"]  =  "+e+"\n";
}
if("INSERT"  ===  c){
	return b+".insert("+a+", "+e+")\n";break;
}break;
case "RANDOM":Blockly.Python.definitions_.import_random = "import random";
...
if("SET"  ===  c){
	return a+(b+"["+d+"]  =  "+e+"\n");}
if("INSERT"  ===  c){
	return a+(b+".insert("+d+", "+e+")\n")}throw"Unhandled combination (lists_setIndex).";break;default:};

Blockly.Python.lists_getSublist = function(a){var b = Blockly.Python.valueToCode(a,"LIST",Blockly.Python.ORDER_MEMBER)  ||  "[]",
c = a.getFieldValue("WHERE1"),d = a.getFieldValue("WHERE2");
switch(c){case "FROM_START":c = Blockly.Python.getAdjustedInt(a,"AT1");"0"  ===  c && (c = "");break;
case "FROM_END":c = Blockly.Python.getAdjustedInt(a,"AT1",1,!0);break;case "FIRST":c = "";break;default:throw"Unhandled option (lists_getSublist)";}
switch(d){case "FROM_START":a = Blockly.Python.getAdjustedInt(a,"AT2",1);break;case "FROM_END":a = 
...
break;case "LAST":a = "";break;default:throw"Unhandled option (lists_getSublist)";}return[b+"["+c+" : "+a+"]",Blockly.Python.ORDER_MEMBER]};
Blockly.Python.lists_sort = function(a){var b = Blockly.Python.valueToCode(a,"LIST",Blockly.Python.ORDER_NONE)  ||  "[]",c = a.getFieldValue("TYPE");
...
"  }","  key_func  =  key_funcs[type]","  list_cpy  =  list(my_list)","  return sorted(list_cpy, key = key_func, reverse = reverse)"])+"("+b+', "'+c+'", '+a+")",
Blockly.Python.ORDER_FUNCTION_CALL]};
Blockly.Python.lists_split = function(a){var b = a.getFieldValue("MODE");
if("SPLIT"  ===  b){
...
else throw"Unknown mode: "+b;return[a,Blockly.Python.ORDER_FUNCTION_CALL]};
...
Blockly.Python.logic_null = function(){return["None",Blockly.Python.ORDER_ATOMIC]};
...
Blockly.isNumber(e))c = parseFloat(c),d = parseFloat(d),
e = Math.abs(parseFloat(e)),0  ===  c%1 && 0  ===  d%1 && 0  ===  e%1?(c  <=  d?(d++,a = 0  ===  c && 1  ===  e?d:c+", "+d,1 !==  e && (a +=  ", "+e)):(d--,a = c+", "+d+", -"+e),
	a = "range("+a+")"):(a = c < d?h():k(),a +=  "("+c+", "+d+", "+e+")");else{var l = function(a,c){if(Blockly.isNumber(a)){a = parseFloat(a);}
	else if(a.match(/^\w+$/)){a = "float("+a+")";}
	else{var d = Blockly.Python.variableDB_.getDistinctName(b+c,Blockly.Variables.NAME_TYPE);
...
Blockly.Python.controls_forEach = function(a){var b = Blockly.Python.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE),
c = Blockly.Python.valueToCode(a,"LIST",Blockly.Python.ORDER_RELATIONAL)  ||  "[]",d = Blockly.Python.statementToCode(a,"DO");
d = Blockly.Python.addLoopTrap(d,a.id)  ||  Blockly.Python.PASS;return"for "+b+" in "+c+":\n"+d};
...
case "SIN":c = "math.sin("+a+" / 180.0 * math.pi)";break;
case "COS":c = "math.cos("+a+" / 180.0 * math.pi)";break;
case "TAN":c = "math.tan("+a+" / 180.0 * math.pi)";break;default:}if(c){return[c,Blockly.Python.ORDER_FUNCTION_CALL];}switch(b){case "ASIN":c = 
...
Blockly.Python.math_number_property = function(a){var b = Blockly.Python.valueToCode(a,"NUMBER_TO_CHECK",Blockly.Python.ORDER_MULTIPLICATIVE)  ||  "0"
,c = a.getFieldValue("PROPERTY");if("PRIME"  ===  c){return Blockly.Python.definitions_.import_math = "import math",Blockly.Python.definitions_.from_numbers_import_Number
 = "from numbers import Number",[Blockly.Python.provideFunction_("math_isPrime",["def "+Blockly.Python.FUNCTION_NAME_PLACEHOLDER_+"(n):",
...
switch(c){case "EVEN":var d = b+" % 2   ===   0";break;
case "ODD":d = b+" % 2   ===   1";break;
case "WHOLE":d = b+" % 1   ===   0";break;
case "POSITIVE":d = b+" > 0";break; 
case "NEGATIVE":d = b+"  <  0";break;
case "DIVISIBLE_BY":a = Blockly.Python.valueToCode(a,"DIVISOR",Blockly.Python.ORDER_MULTIPLICATIVE);break;default:
...
Blockly.Python.math_random_int = function(){Blockly.Python.definitions_.import_random = "import random";var b = Blockly.Python.valueToCode(a,"FROM",Blockly.Python.ORDER_NONE)  ||  "0";a = Blockly.Python.valueToCode(a,"TO",Blockly.Python.ORDER_NONE)  ||  "0";return["random.randint("+b+", "+a+")",Blockly.Python.ORDER_FUNCTION_CALL]};Blockly.Python.math_random_float = function(){Blockly.Python.definitions_.import_random = "import random";return["random.random()",Blockly.Python.ORDER_FUNCTION_CALL]};Blockly.Python.procedures = {};
Blockly.Python.procedures_defreturn = function(a){for(var b = [],c,d = a.workspace.getAllVariables()  ||  [],e = 0;c = d[e];e++){c = c.name,-1  ===  a.arguments_.indexOf(c) && b.push(Blockly.Python.variableDB_.getName(c,Blockly.Variables.NAME_TYPE));}b = b.length?Blockly.Python.INDENT+"global "+b.join(", ")+"\n":"";d = Blockly.Python.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE);c = Blockly.Python.statementToCode(a,"STACK");Blockly.Python.STATEMENT_PREFIX && (e = a.id.replace(/\$/g,"$$$$"),c = Blockly.Python.prefixLines(Blockly.Python.STATEMENT_PREFIX.replace(/%1/g,
"'"+e+"'"),Blockly.Python.INDENT)+c);Blockly.Python.INFINITE_LOOP_TRAP && (c = Blockly.Python.INFINITE_LOOP_TRAP.replace(/%1/g,'"'+a.id+'"')+c);var f = Blockly.Python.valueToCode(a,"RETURN",Blockly.Python.ORDER_NONE)  ||  "";f?f = Blockly.Python.INDENT+"return "+f+"\n":c  ||  (c = Blockly.Python.PASS);var g = [];for(var e = 0;e < a.arguments_.length;e++){g[e] = Blockly.Python.variableDB_.getName(a.arguments_[e],Blockly.Variables.NAME_TYPE);}b = "def "+d+"("+g.join(", ")+"):\n"+b+c+f;b = Blockly.Python.scrub_(a,b);Blockly.Python.definitions_["%"+
d] = b;return null};
Blockly.Python.procedures_defnoreturn = Blockly.Python.procedures_defreturn;
Blockly.Python.procedures_callreturn = function(a){for(var c = [],d = 0;d < a.arguments_.length;d++){c[d] = Blockly.Python.valueToCode(a,"ARG"+d,Blockly.Python.ORDER_NONE)  ||  "None";}return[Blockly.Python.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE) +"("+c.join(", ")+")",Blockly.Python.ORDER_FUNCTION_CALL]};
Blockly.Python.procedures_callnoreturn = function(a){for(var c = [],d = 0;d < a.arguments_.length;d++){c[d] = Blockly.Python.valueToCode(a,"ARG"+d,Blockly.Python.ORDER_NONE)  ||  "None"};return Blockly.Python.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE)+"("+c.join(", ")+")\n"};
...
Blockly.Python.text_join = function(a){switch(a.itemCount_){case 0:return["''",Blockly.Python.ORDER_ATOMIC];case 1:return["str("+(Blockly.Python.valueToCode(a,"ADD0",Blockly.Python.ORDER_NONE)  ||  "''")+")",Blockly.Python.ORDER_FUNCTION_CALL];case 2:var b = Blockly.Python.valueToCode(a,"ADD0",Blockly.Python.ORDER_NONE)  ||  "''";a = Blockly.Python.valueToCode(a,"ADD1",Blockly.Python.ORDER_NONE)  ||  "''";return["str("+b+") + str("+a+")",Blockly.Python.ORDER_ADDITIVE];default:b = [];for(var c = 0;c < a.itemCount_;c++){b[c] = Blockly.Python.valueToCode(a,"ADD"+c,Blockly.Python.ORDER_NONE)  ||  "''";}a = Blockly.Python.variableDB_.getDistinctName("x",Blockly.Variables.NAME_TYPE);a = "''.join([str("+a+") for "+a+" in ["+b.join(", ")+"]])";return[a,Blockly.Python.ORDER_FUNCTION_CALL]}};
Blockly.Python.text_append = function(a){var b = Blockly.Python.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);a = Blockly.Python.valueToCode(a,"TEXT",Blockly.Python.ORDER_NONE)  ||  "''";return b+"  =  str("+b+") + str("+a+")\n"};
...
Blockly.Python.text_charAt = function(a){var b = a.getFieldValue("WHERE")  ||  "FROM_START",c = Blockly.Python.valueToCode(a,"VALUE",Blockly.Python.ORDER_MEMBER)  ||  "''";switch(b){case "FIRST":return[c+"[0]",Blockly.Python.ORDER_MEMBER];
case "LAST":return[c+"[-1]",Blockly.Python.ORDER_MEMBER];break;
case "FROM_START":return a = Blockly.Python.getAdjustedInt(a,"AT"),[c+"["+a+"]",Blockly.Python.ORDER_MEMBER];break;
case "FROM_END":return a = Blockly.Python.getAdjustedInt(a,"AT",1,!0),[c+"["+a+"]",Blockly.Python.ORDER_MEMBER];break;
case "RANDOM":return Blockly.Python.definitions_.import_random = "import random",[Blockly.Python.provideFunction_("text_random_letter",["def "+Blockly.Python.FUNCTION_NAME_PLACEHOLDER_+"(text):","  x  =  int(random.random() * len(text))","  return text[x];"])+"("+c+")",Blockly.Python.ORDER_FUNCTION_CALL];break;default:throw"Unhandled option (text_charAt).";};
