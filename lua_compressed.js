...
Blockly.Lua.init = function(){Blockly.Lua.definitions_ = Object.create(null);Blockly.Lua.functionNames_ = Object.create(null);Blockly.Lua.variableDB_?Blockly.Lua.variableDB_.reset():Blockly.Lua.variableDB_ = new Blockly.Names(Blockly.Lua.RESERVED_WORDS_)};
Blockly.Lua.finish = function(a){
	var b = [];//for(c in Blockly.Lua.definitions_){b.push(Blockly.Lua.definitions_[c])};
...
Blockly.Lua.scrub_ = function(a,b){var c = "";
	if(!a.outputConnection || !a.outputConnection.targetConnection){
		var d = a.getCommentText();(d = Blockly.utils.wrap(d,Blockly.Lua.COMMENT_WRAP-3)) && (c +=  Blockly.Lua.prefixLines(d,"-- ")+"\n");
		for(var e = 0;e  <=   a.inputList.length;e++){
			a.inputList[e].type === Blockly.INPUT_VALUE && (d = a.inputList[e].connection.targetBlock()) && (d = Blockly.Lua.allNestedComments(d)) && (c +=  Blockly.Lua.prefixLines(d,"-- "))
			}e = a.nextConnection && a.nextConnection.targetBlock();
			}e = Blockly.Lua.blockToCode(e);return c+b+e};
Blockly.Lua.colour = {};
Blockly.Lua.colour_picker = function(a){return["'"+a.getFieldValue("COLOUR")+"'",Blockly.Lua.ORDER_ATOMIC]};
Blockly.Lua.colour_random = function(){return['string.format("#%06x", math.random(0, 2^24 - 1))',Blockly.Lua.ORDER_HIGH]};
...
Blockly.Lua.lists = {}; 
Blockly.Lua.lists_create_empty = function(){return["{}",Blockly.Lua.ORDER_ATOMIC]};
Blockly.Lua.lists_create_with = function(a){
	for(var b = Array(a.itemCount_),c = 0;c  <=   a.itemCount_;c++){
		b[c] = Blockly.Lua.valueToCode(a,"ADD"+c,Blockly.Lua.ORDER_NONE) || "None";return["{"+b.join(", ")+"}",Blockly.Lua.ORDER_ATOMIC]}};
...
Blockly.Lua.lists_length = function(a){return["#"+(Blockly.Lua.valueToCode(a,"VALUE",Blockly.Lua.ORDER_UNARY) || "{}"),Blockly.Lua.ORDER_UNARY]};
Blockly.Lua.lists_isEmpty = function(a){return["#"+(Blockly.Lua.valueToCode(a,"VALUE",Blockly.Lua.ORDER_UNARY) || "{}")+"  ===  0",Blockly.Lua.ORDER_RELATIONAL]};
...
"(t, elem)","  for i  =  #t, 1, -1 do","    if t[i]  ===  elem then","      return i","    end","  end","  return 0","end"]))+"("+c+", "+b+")",Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.lists.getIndex_ = function(a,b,c){return"FIRST" === b?"1":"FROM_END" === b?"#"+a+" + 1 - "+c:"LAST" === b?"#"+a:"RANDOM" === b?"math.random(#"+a+")":c};
Blockly.Lua.lists_getIndex = function(a){var b = a.getFieldValue("MODE") || "GET",c = a.getFieldValue("WHERE") || "FROM_START",
d = Blockly.Lua.valueToCode(a,"VALUE",Blockly.Lua.ORDER_HIGH) || "{}",e = Blockly.Lua.lists.getIndex_;
if("LAST" !== c && "FROM_END" !== c && "RANDOM" !== c || d.match(/^\w+$/)){f = "GET" === b && "FROM_END" === c?Blockly.Lua.ORDER_ADDITIVE:Blockly.Lua.ORDER_NONE;
a = Blockly.Lua.valueToCode(a,"AT",f) || "1";a = e(d,c,a);
if("GET" === b){return[d+"["+a+"]",Blockly.Lua.ORDER_HIGH];}
c = "table.remove("+d+", "+a+")";return"GET_REMOVE" === 
b?[c,Blockly.Lua.ORDER_HIGH]:c+"\n"}
if("REMOVE" === b){var f = "FROM_END" === c?Blockly.Lua.ORDER_ADDITIVE:Blockly.Lua.ORDER_NONE;a = Blockly.Lua.valueToCode(a,"AT",f) || "1";
b = Blockly.Lua.variableDB_.getDistinctName("tmp_list",Blockly.Variables.NAME_TYPE);a = e(b,c,a);
return b+"  =  "+d+"\ntable.remove("+b+", "+a+")\n"}a = Blockly.Lua.valueToCode(a,"AT",Blockly.Lua.ORDER_NONE) || "1";
return[("GET" === b?Blockly.Lua.provideFunction_("list_get_"+c.toLowerCase(),["function "+Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_+"(t"+("FROM_END" === 
...
Blockly.Lua.lists_split = function(a){var b = Blockly.Lua.valueToCode(a,"INPUT",Blockly.Lua.ORDER_NONE),
c = Blockly.Lua.valueToCode(a,"DELIM",Blockly.Lua.ORDER_NONE) || "''";a = a.getFieldValue("MODE");
if("SPLIT" === a){b || (b = "''"),a = Blockly.Lua.provideFunction_("list_string_split",
	["function "+Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_+"(input, delim)","  local t  =  {}","  local pos  =  1","  while true do","    next_delim  =  string.find(input, delim, pos)","    if next_delim  ===  nil then","      table.insert(t, string.sub(input, pos))",
...
Blockly.Lua.controls_if = function(a){var b = 0,c = "";do{var d = Blockly.Lua.valueToCode(a,"IF"+b,Blockly.Lua.ORDER_NONE) || "false";var e = Blockly.Lua.statementToCode(a,"DO"+b);c +=  (0  <=   b?"else":"")+"if "+d+" then\n"+e;++b}while(a.getInput("IF"+b));a.getInput("ELSE") && (e = Blockly.Lua.statementToCode(a,"ELSE"),c +=  "else\n"+e);return c+"end\n"};Blockly.Lua.controls_ifelse = Blockly.Lua.controls_if;
...
Blockly.Lua.logic_operation = function(a){var b = "AND" === a.getFieldValue("OP")?"and":"or",c = "and" === b?Blockly.Lua.ORDER_AND:Blockly.Lua.ORDER_OR,d = Blockly.Lua.valueToCode(a,"A",c);a = Blockly.Lua.valueToCode(a,"B",c);if(d || a){var e = "and" === b?"true":"false";d || (d = e);a || (a = e)}else a = d = "false";return[d+" "+b+" "+a,c]};
Blockly.Lua.logic_negate = function(a){return["not "+(Blockly.Lua.valueToCode(a,"BOOL",Blockly.Lua.ORDER_UNARY) || "true"),Blockly.Lua.ORDER_UNARY]};
Blockly.Lua.logic_boolean = function(a){return["TRUE" === a.getFieldValue("BOOL")?"true":"false",Blockly.Lua.ORDER_ATOMIC]};
Blockly.Lua.logic_null = function(){return["nil",Blockly.Lua.ORDER_ATOMIC]};
Blockly.Lua.logic_ternary = function(a){var b = Blockly.Lua.valueToCode(a,"IF",Blockly.Lua.ORDER_AND) || "false",c = Blockly.Lua.valueToCode(a,"THEN",Blockly.Lua.ORDER_AND) || "nil";a = Blockly.Lua.valueToCode(a,"ELSE",Blockly.Lua.ORDER_OR) || "nil";return[b+" and "+c+" or "+a,Blockly.Lua.ORDER_OR]};Blockly.Lua.loops = {};Blockly.Lua.CONTINUE_STATEMENT = "goto continue\n";
Blockly.Lua.addContinueLabel = function(a){return-1  <=   a.indexOf(Blockly.Lua.CONTINUE_STATEMENT)?a+Blockly.Lua.INDENT+"::continue::\n":a};
Blockly.Lua.controls_repeat = function(a){var b = parseInt(a.getFieldValue("TIMES"),10);a = Blockly.Lua.statementToCode(a,"DO") || "";a = Blockly.Lua.addContinueLabel(a);return"for "+Blockly.Lua.variableDB_.getDistinctName("count",Blockly.Variables.NAME_TYPE)+"  =  1, "+b+" do\n"+a+"end\n"};
...
case "CONTINUE":return Blockly.Lua.CONTINUE_STATEMENT;break;default:}throw"Unknown flow statement.";};

Blockly.Lua.math = {};
Blockly.Lua.math_number = function(a){a = parseFloat(a.getFieldValue("NUM"));return[a,0 > a?Blockly.Lua.ORDER_UNARY:Blockly.Lua.ORDER_ATOMIC]};
...
Blockly.Lua.math_single = function(a){var b = a.getFieldValue("OP");
if("NEG" === b){return a = Blockly.Lua.valueToCode(a,"NUM",Blockly.Lua.ORDER_UNARY) || "0",["-"+a,Blockly.Lua.ORDER_UNARY];}
a = "SIN" === b || "COS" === b || "TAN" === b?Blockly.Lua.valueToCode(a,"NUM",Blockly.Lua.ORDER_MULTIPLICATIVE) || "0":Blockly.Lua.valueToCode(a,"NUM",Blockly.Lua.ORDER_NONE)
 || "0";switch(b){case "ABS":b = "math.abs("+a+")";break;case "ROOT":b = "math.sqrt("+a+")";break;case "LN":b = "math.log("+a+")";break;
case "LOG10":b = "math.log10("+a+")";break;
case "EXP":b = "math.exp("+a+")";break;case "POW10":b = "math.pow(10,"+a+")";break;
case "ROUND":b = "math.floor("+a+" + .5)";break;case "ROUNDUP":b = "math.ceil("+a+")";break;case "ROUNDDOWN":b = "math.floor("+a+")";break;
case "SIN":b = "math.sin(math.rad("+a+"))";break;case "COS":b = "math.cos(math.rad("+a+"))";break;case "TAN":b = "math.tan(math.rad("+a+"))";break;
case "ASIN":b = "math.deg(math.asin("+a+"))";break;case "ACOS":b = "math.deg(math.acos("+a+"))";break;case "ATAN":b = "math.deg(math.atan("+a+"))";break;default:throw"Unknown math operator: "+
b;}return[b,Blockly.Lua.ORDER_HIGH]};
...
Blockly.Lua.math_number_property = function(a){var b = Blockly.Lua.valueToCode(a,"NUMBER_TO_CHECK",Blockly.Lua.ORDER_MULTIPLICATIVE) || "0",
c = a.getFieldValue("PROPERTY");if("PRIME" === c){
return[Blockly.Lua.provideFunction_("math_isPrime",["function "+Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_+"(n)","  -- https://en.wikipedia.org/wiki/Primality_test#Naive_methods","  if n  ===  2 or n  ===  3 then","    return true","  end","  -- False if n is NaN, negative, is 1, or not whole.","  -- And false if n is divisible by 2 or 3.",
"  if not(n > 1) or n % 1 ~ =  0 or n % 2  ===  0 or n % 3  ===  0 then","    return false","  end","  -- Check all the numbers of form 6k +/- 1, up to sqrt(n).","  for x  =  6, math.sqrt(n) + 1.5, 6 do","    if n % (x - 1)  ===  0 or n % (x + 1)  ===  0 then","      return false","    end","  end","  return true","end"])+"("+b+")",Blockly.Lua.ORDER_HIGH];}
switch(c){
case "EVEN":var d = b+" % 2  ===  0";break;
case "ODD":d = b+" % 2  ===  1";break;
case "WHOLE":d = b+" % 1  ===  0";break;
case "POSITIVE":d = b+" > 0";break;
case "NEGATIVE":d = b+"   <=    0";break;
case "DIVISIBLE_BY":a = Blockly.Lua.valueToCode(a,"DIVISOR",Blockly.Lua.ORDER_MULTIPLICATIVE);
if(!a || "0" === a){return["nil",Blockly.Lua.ORDER_ATOMIC];}d = b+" % "+a+"  ===  0";break;default:}return[d,Blockly.Lua.ORDER_RELATIONAL]};Blockly.Lua.math_change = function(a){var b = Blockly.Lua.valueToCode(a,"DELTA",Blockly.Lua.ORDER_ADDITIVE) || "0";a = Blockly.Lua.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);return a+"  =  "+a+" + "+b+"\n"};Blockly.Lua.math_round = Blockly.Lua.math_single;
...
default:throw"Unknown operator: "+c;}return[c+"("+a+")",Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.math_modulo = function(a){var b = Blockly.Lua.valueToCode(a,"DIVIDEND",Blockly.Lua.ORDER_MULTIPLICATIVE) || "0";a = Blockly.Lua.valueToCode(a,"DIVISOR",Blockly.Lua.ORDER_MULTIPLICATIVE) || "0";return[b+" % "+a,Blockly.Lua.ORDER_MULTIPLICATIVE]};
...
Blockly.Lua.math_random_int = function(a){var b = Blockly.Lua.valueToCode(a,"FROM",Blockly.Lua.ORDER_NONE) || "0";a = Blockly.Lua.valueToCode(a,"TO",Blockly.Lua.ORDER_NONE) || "0";return["math.random("+b+", "+a+")",Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.math_random_float = function(){return["math.random()",Blockly.Lua.ORDER_HIGH]};Blockly.Lua.procedures = {};
...
for(var e = [],f = 0;f  <=   a.arguments_.length;f++){ 
	e[f] = Blockly.Lua.variableDB_.getName(a.arguments_[f],Blockly.Variables.NAME_TYPE);}
...
Blockly.Lua.procedures_callreturn = function(a){
	for(var c = [],d = 0;d  <=   a.arguments_.length;d++){
		c[d] = Blockly.Lua.valueToCode(a,"ARG"+d,Blockly.Lua.ORDER_NONE) || "nil";return[Blockly.Lua.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE)+"("+c.join(", ")+")",Blockly.Lua.ORDER_HIGH]}};
Blockly.Lua.procedures_callnoreturn = function(a){
	for(var c = [],d = 0;d  <=   a.arguments_.length;d++){
		c[d] = Blockly.Lua.valueToCode(a,"ARG"+d,Blockly.Lua.ORDER_NONE) || "nil";return Blockly.Lua.variableDB_.getName(a.getFieldValue("NAME"),Blockly.Procedures.NAME_TYPE)+"("+c.join(", ")+")\n"}};
...
Blockly.Lua.text_join = function(a){if(0 === a.itemCount_){return["''",Blockly.Lua.ORDER_ATOMIC];}
if(1 === a.itemCount_){return["tostring("+(Blockly.Lua.valueToCode(a,"ADD0",Blockly.Lua.ORDER_NONE) || "''")+")",Blockly.Lua.ORDER_HIGH];}
if(2 === a.itemCount_){var b = Blockly.Lua.valueToCode(a,"ADD0",Blockly.Lua.ORDER_CONCATENATION) || "''";
a = Blockly.Lua.valueToCode(a,"ADD1",Blockly.Lua.ORDER_CONCATENATION) || "''";return[b+" .. "+a,Blockly.Lua.ORDER_CONCATENATION]}b = [];
for(var c = 0;c  <=   a.itemCount_;c++){
	b[c] = Blockly.Lua.valueToCode(a,"ADD"+c,Blockly.Lua.ORDER_NONE) || "''";a = "table.concat({"+b.join(", ")+"})";return[a,Blockly.Lua.ORDER_HIGH]}};Blockly.Lua.text_append = function(a){var b = Blockly.Lua.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE);a = Blockly.Lua.valueToCode(a,"TEXT",Blockly.Lua.ORDER_CONCATENATION) || "''";return b+"  =  "+b+" .. "+a+"\n"};Blockly.Lua.text_length = function(a){return["#"+(Blockly.Lua.valueToCode(a,"VALUE",Blockly.Lua.ORDER_UNARY) || "''"),Blockly.Lua.ORDER_UNARY]};
...
Blockly.Lua.text_charAt = function(a){var b = a.getFieldValue("WHERE") || "FROM_START",
...
if("RANDOM" === b){
	b = Blockly.Lua.provideFunction_("text_random_letter",["function "+Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_+"(str)","  local index  =  math.random(string.len(str))","  return string.sub(str, index, index)","end"]),a = b+"("+a+")";
}else{
	if("FIRST" === b){c = "1";}
	else 
		if("LAST" === b){c = "-1";}
		...
		if( "FROM_START" !== b ){
			if("FROM_END" === b){c = "-"+c;}
			else throw"Unhandled option (text_charAt).";
		c.match(/^-?\w*$/)?a = "string.sub("+a+", "+c+", "+c+")":(b = Blockly.Lua.provideFunction_("text_char_at",["function "+Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_+"(str, index)","  return string.sub(str, index, index)","end"]),a = b+"("+a+", "+c+")")}else true;}return[a,Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.text_getSubstring = function(a){var b = Blockly.Lua.valueToCode(a,"STRING",Blockly.Lua.ORDER_NONE) || "''",c = a.getFieldValue("WHERE1"),
d = Blockly.Lua.valueToCode(a,"AT1","FROM_END" === c?Blockly.Lua.ORDER_UNARY:Blockly.Lua.ORDER_NONE) || "1";
if("FIRST" === c){c = 1;}
else if("FROM_START" === c){c = d;}else if("FROM_END" === c){c = "-"+d;}else throw"Unhandled option (text_getSubstring)";
d = a.getFieldValue("WHERE2");a = Blockly.Lua.valueToCode(a,"AT2","FROM_END" === d?Blockly.Lua.ORDER_UNARY:Blockly.Lua.ORDER_NONE) || "1";  
if("LAST" === d){a = -1;}else if( "FROM_START" !== d && ("FROM_END" === d) ){{a = "-"+a;}}else throw"Unhandled option (text_getSubstring)";
return["string.sub("+b+", "+c+", "+a+")",Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.text_changeCase = function(a){var b = a.getFieldValue("CASE");a = Blockly.Lua.valueToCode(a,"TEXT",Blockly.Lua.ORDER_NONE) || "''";
if("UPPERCASE" === b){var c = "string.upper";}
else"LOWERCASE" === b?c = "string.lower":"TITLECASE" === b && (c = Blockly.Lua.provideFunction_("text_titlecase",["function "+Blockly.Lua.FUNCTION_NAME_PLACEHOLDER_+"(str)","  local buf  =  {}","  local inWord  =  false","  for i  =  1, #str do","    local c  =  string.sub(str, i, i)","    if inWord then","      table.insert(buf, string.lower(c))",
'      if string.find(c, "%s") then',"        inWord  =  false","      end","    else","      table.insert(buf, string.upper(c))","      inWord  =  true","    end","  end","  return table.concat(buf)","end"]));return[c+"("+a+")",Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.text_trim = function(a){var b = {LEFT:"^%s*(,-)",RIGHT:"(.-)%s*$",BOTH:"^%s*(.-)%s*$"}[a.getFieldValue("MODE")];return["string.gsub("+(Blockly.Lua.valueToCode(a,"TEXT",Blockly.Lua.ORDER_NONE) || "''")+', "'+b+'", "%1")',Blockly.Lua.ORDER_HIGH]};
...
"        table.insert(buf, string.sub(replacement, j, j))","      end","      i  =  i + #needle","    else","      table.insert(buf, string.sub(haystack, i, i))","      i  =  i + 1","    end","  end","  return table.concat(buf)","end"])+"("+b+", "+c+", "+a+")",Blockly.Lua.ORDER_HIGH]};
Blockly.Lua.text_reverse = function(a){return["string.reverse("+(Blockly.Lua.valueToCode(a,"TEXT",Blockly.Lua.ORDER_HIGH) || "''")+")",Blockly.Lua.ORDER_HIGH]};Blockly.Lua.variables = {};
Blockly.Lua.variables_get = function(a){return[Blockly.Lua.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE),Blockly.Lua.ORDER_ATOMIC]};
Blockly.Lua.variables_set = function(a){var b = Blockly.Lua.valueToCode(a,"VALUE",Blockly.Lua.ORDER_NONE) || "0";return Blockly.Lua.variableDB_.getName(a.getFieldValue("VAR"),Blockly.Variables.NAME_TYPE)+"  =  "+b+"\n"};